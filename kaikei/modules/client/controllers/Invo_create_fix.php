...
class Invo_create_fix extends MY_Controller
{

    /*
     *  請求書データの一括作成処理（固定報酬関連）
     */
    ...
    // 月額請求書データ 一括作成：固定報酬（SEO固定/固定/保守/アフィ/広告/その他）
    public function fix_cal()
    {
        ...
        // バリデーション・チェック
        if ($input_post['_submit'] == "save_all")
        {
        ...
        if ($this->form_validation->run() == TRUE)
        {
            ...
            {
                $cm_list = $this->cm->get_ivlist($opt_timing['fix'], $input_post['iv_cm_seq01']);
            } else {
                $cm_list = $this->cm->get_ivlist($opt_timing['fix']);
            }
            ...
            foreach($cm_list as $key => $value)
            {
                ...
                foreach($get_pj_list as $key => $val)
                {
                    ...
                    // 契約期間チェック
                    $_project_date = $this->lib_invoice->issue_project_date($input_post['iv_issue_yymm'], $val['pj_start_date'], $val['pj_end_date'], $value['cm_collect']);
                ...
                if (count($list_detail))
                {
                    ...
                    $set_data_iv['iv_sales_yymm'] = $_collect_date['sales_yymm'];                               // 売上月度
...
//                  if (count($get_iv_data) == 0)
            ...
            if ($this->db->trans_status() === FALSE)
            {
                ...
                $this->smarty->assign('mess', "<font color=blue>作成が正常に終了しました。</font>");
            }

        } else {
            $this->smarty->assign('mess', "<font color=red>固定関連請求データ作成：項目入力エラー。</font>");
        }
    ...
    public function paydate_chg()
    {
        ...
        if ($this->form_validation->run() == TRUE)
        {
            ...
            // 該当振込期日のデータを抽出
            $get_paydata = $this->iv->get_iv_paydate($input_post['iv_issue_yymm'], $input_post['iv_pay_date01']);

            foreach ($get_paydata as $key => $value)
            {

                $set_paydate['iv_seq']      = $get_paydata[$key]['iv_seq'];
                $set_paydate['iv_pay_date'] = $input_post['iv_pay_date02'];

                $this->iv->update_invoice($set_paydate);

            }

            $this->smarty->assign('mess', "<font color=blue>更新が正常に終了しました。</font>");

        } else {
            $this->smarty->assign('mess', "<font color=red>振込期日の一括変更：項目入力エラー。</font>");
        }
...
//      $this->smarty->assign('options_date_res', $opt_date_fix);   // テスト用
