...
class Pdf_shokai extends MY_Controller
{
...
//          $this->load->library('lib_shokai');
            ...
            // 支払通知書データの取得
            $get_skf_data[0] = $this->sk->get_skf_seq($input_post['skf_seq']);

            // 売上先会社データの取得
            $get_skc_data = $this->sk->get_company_sk_seq($input_post['skf_seq']);

            // バリデーション・チェック
            $this->_set_validation();

            // 「ステータス」を更新（：発行済）。備考やメモ欄は更新しない、更新する場合は「更新」ボタンから。
            $get_skf_data[0][0]['skf_status'] = 1;

            // 紹介情報の更新
            unset($get_skf_data[0][0]['skf_create_date']);
            unset($get_skf_data[0][0]['skf_update_date']);
            $this->sk->update_shokai_fee($get_skf_data[0][0]);
            ...
            // PDFライブラリ呼出
            $this->load->library('lib_pdf_shokai');
            $this->lib_pdf_shokai->create_pdf($get_skf_data, $get_skc_data, $pdflist_path, $base_path);


        } else {
            redirect('/shokaifeelist/');
        }
        ...
        if ($input_post['_submit'] == 'submit')
        {
            ...
            if (count($input_post) >= 3)
            {
                ...
                foreach ($input_post as $key => $val)
                {
                    ...
                    {
                        log_message('error', 'CLIENT::[Pdf_create -> pdf_one()]：支払通知書PDF 個別作成処理 トランザクションエラー');
                    }
