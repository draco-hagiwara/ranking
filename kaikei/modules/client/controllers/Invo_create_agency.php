...
class Invo_create_agency extends MY_Controller
{
    ...
    public function agency_cal()
    {
        ...
        if ($this->form_validation->run() == TRUE)
        {
            ...
            {
                $cm_list = $this->cm->get_ivlist($opt_timing['agency'], $input_post['iv_cm_seq03']);
            } else {
                $cm_list = $this->cm->get_ivlist($opt_timing['agency']);
            }
            ...
            foreach($cm_list as $key => $value)
            {
                ...
                if ($input_post['_submit'] == "save_oly")
                {
                    $get_pj_list = $this->cm->get_ivlist_agency($value['cm_seq'], TRUE);
                    $_invo_class = "C";
                } else {
                    $get_pj_list = $this->cm->get_ivlist_agency($value['cm_seq'], FALSE);
                    $_invo_class = "B";
                }
                ...
                foreach($get_pj_list as $key => $val)
                {
                    ...
                    if (($_project_date['str'] <= $_project_date['now']) && ($_project_date['now'] <= $_project_date['end']))
                    {
                        ...
                        /*
                         * 売上種別の判定：全て混合で出力するとの仕様なので、判定が面倒なので初期値を設定していく。
                ...
                if (count($list_detail))
                {
                    ...
                    $set_data_iv['iv_agency_flg']     = $value['cm_agency_flg'];                                // 代理店有無フラグ
...
//                  if (count($get_iv_data) == 0)
                    ...
                    foreach($list_detail as $key => $val)
                    {
                        ...
                        $set_data_ivd['ivd_item_cmseq']     = $val['pj_cm_seq'];                                // 請求項目=顧客cm_seq (代理店請求のみ)
            ...
            {
                log_message('error', 'CLIENT::[Invo_create_agency -> agency_cal()]：代理店関連請求書一括作成処理 トランザクションエラー');
            } else {
                $this->smarty->assign('mess', "<font color=blue>作成が正常に終了しました。</font>");
            }

        } else {
            $this->smarty->assign('mess', "<font color=red>代理店関連請求データ作成：項目入力エラー。</font>");
        }
