...
class Lib_searchrank_data
{
	...
	public static function create_rank_graph($kw_list, $term)
	{
		...
		// グラフ用データ取得
		$_tmp_comp_cnt = array();												// キーワード一覧：ランク数字
		$_line_cnt     = 0;														// キーワード一覧：行数カウント
		foreach ($kw_list as $key => $value)
		{
			$_tmp_comp_cnt[$_line_cnt] = $CI->lib_searchrank_data->_get_rank_graph($value['kw_seq'], $term, $_line_cnt, $set_comp_date);
			$_line_cnt++;
		}

		// ランキング順位と前日差分カウントを返す
		return $_tmp_comp_cnt;
	...
	public static function _get_rank_graph($kw_seq, $term, $line_cnt, $set_comp_date)
	{
		...
		foreach ($get_kw_pair as $key => $value)
		{
			...
			$_tmp_comp_cnt[$line_cnt][$i] = $CI->lib_searchrank_data->_jqplot_data($value, $_start_term, $_end_term, $_date_cnt, $line_cnt, $set_comp_date);
