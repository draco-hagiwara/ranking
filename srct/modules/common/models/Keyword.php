...
class Keyword extends CI_Model
{
     ...
     * @return   array()
     */
    public function get_kw_rootdomainlist($get_item, $tmp_per_page=0, $tmp_offset=0, $client_no)
    {
...
//     	// ORDER BY
//     	if (empty($get_item['free_sort']))
//     	{
//     		$set_orderby = NULL;
//     	} else {
//     		$set_orderby = $get_item['free_sort'];
//     	}

    	// 対象情報の取得
    	$kw_list = $this->_select_rootdomainlist($get_item['free_keyword'], $set_select, $set_orderby, $client_no, $get_item['free_sort_id']);
     ...
     * @return   array()
     */
    public function _select_rootdomainlist($free_kw, $set_select, $set_orderby, $client_no, $free_sort_id=NULL)
    {
...
//         				print("rootdomain_list<br>");
//             	    	print_r($rootdomain_list);
// //             	    	var_dump($rootdomain_list);
//             	    	print("<br><br>");
...
//         	print("<br><br>");
        ...
        return array($kw_list, $rootdomain_countall, $kw_countall);
     ...
     * @return   array()
     */
    public function get_kw_grouplist($get_post, $tmp_per_page=0, $tmp_offset=0, $client_no)
    {
    	...
    	// 対象情報の取得
    	$kw_list = $this->_select_grouplist($get_post, $set_orderby, $client_no);
//     	$kw_list = $this->_select_grouplist($get_post['free_keyword'], $set_orderby, $client_no);
     ...
     * @return   array()
     */
    public function _select_grouplist($get_post, $set_orderby, $client_no)
    {

    	$free_kw  = $get_post['free_keyword'];
    	...
    	// ORDER BY文 作成
    	$sql1 .= ' ORDER BY T3.gt_seq ' .  $set_orderby;
...
//     	            	    	print("<br><br>");
    	...
    	if (!empty($free_kw))
    	{
    		...
    		// 一覧からのグループ指定の場合
    		if (isset($get_post['kw_group']))
    		{
    			$sql2 .= ' AND kw_group = \'' . $get_post['kw_group'] . '\'';
    		}
    	}
    	...
    	if (!empty($gt_list))
    	{
    		...
    		foreach ($gt_list as $key => $val)
    		{
    			...
    			{
    				$_tmp_rootdomain = $val['gt_name'];
    			} else {
    				$_tmp_rootdomain = $val['kw_group'];
    			}

    			if ($tmp_firstitem === FALSE)
    			{
    				$sql2 .= ' AND ( kw_group = \'' . $_tmp_rootdomain . '\'';
    				$tmp_firstitem = TRUE;
    			} else {
    				$sql2 .= ' OR kw_group = \'' . $_tmp_rootdomain . '\'';
    			}
    		}
    		$sql2 .= ' ) ';
...
//     		        	    		        print("<br><br>");
    ...
    public function get_kw_info($setdata)
    {
    	...
    	';
			    	 ...
			    	 //AND kw_domain = \'' . $setdata['kw_domain'] . '\'
...
//         return array($kw_list, $kw_countall);
     ...
     * @return   array()
     */
    public function get_kw_grouplist_xx($get_post, $tmp_per_page, $tmp_offset=0)
    {
    	...
    	// 対象クアカウントメンバーの取得
    	$kw_list = $this->_select_grouplist_xx($set_select, $set_select_like, $set_orderby, $tmp_per_page, $tmp_offset);
     ...
     * @return   array()
     */
    public function _select_grouplist_xx($set_select, $set_select_like, $set_orderby, $tmp_per_page, $tmp_offset=0)
    {
     ...
     * @return   array()
     */
    public function get_kw_rootdomainlist_xx($get_post, $tmp_per_page, $tmp_offset=0)
    {
    	...
    	// 対象クアカウントメンバーの取得
    	$kw_list = $this->_select_rootdomainlist_xx($set_select, $set_select_like, $set_orderby, $tmp_per_page, $tmp_offset);
     ...
     * @return   array()
     */
    public function _select_rootdomainlist_xx($set_select, $set_select_like, $set_orderby, $tmp_per_page, $tmp_offset=0)
    {
...
// 		print($sql);
     ...
     * @return   array()
     */
    public function get_csvdl_reportxxxxxx($get_post, $kw_seq01, $kw_seq02=NULL, $kw_seq03=NULL)
    {
    	...
    	$sql = 'SELECT
    			...
    			if (isset($kw_seq03))
    			{
    				$sql .= '(kw_seq = ' . $kw_seq01 . ' OR kw_seq = '. $kw_seq02 . ' OR kw_seq = '. $kw_seq03 . ') ';
    			} elseif (isset($kw_seq02)) {
    			...
    			$sql .= ' ORDER BY kw_searchengine ASC, kw_device ASC, T1.rk_seq ASC';
